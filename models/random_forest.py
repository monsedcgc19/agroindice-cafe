"""
random_forest.py — Random Forest para predecir NDVI y EVI (ventana MODIS
actual) a partir de clima ERA5/FLDAS rezagado. Corre ambas variables Y
(`ndvi`, `evi`) en una sola ejecución;
...
Output:
  ...
  - results/grid_search_raw/random_forest_ndvi.csv
  - results/grid_search_raw/random_forest_evi.csv
...
Metodología:
  - Split cronológico único (no por y_variable): 85% train_val / 15%
    ...
    (evita fuga temporal: cada fold de validación es siempre posterior a su
    fold de entrenamiento).
  - Valores NaN en features (p. ej. et_mm_lag1 en la primera ventana de cada
    región, ver diccionario_datos.md) no se imputan: RandomForestRegressor
    de scikit-learn soporta NaN nativamente en el split de árboles desde la
    versión 1.4.
"""
...
def run_for_target(y_variable, train_val, test_final, fecha_corte):
    ...
    log_run(
        ...
        rmse=rmse_b,
        pearson=pearson_b,
        notas="RandomForestRegressor con hiperparámetros por defecto",
    )
    ...
    cv_results_path = GRID_SEARCH_DIR / f"random_forest_{y_variable}.csv"
...
def print_side_by_side(resumenes):
    print("\n" + "=" * 60)
    print("Comparación lado a lado: ndvi vs. evi (random_forest)")
    print("=" * 60)
